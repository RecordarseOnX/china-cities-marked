...
function App() {
  ...
  const handleExportPDF = () => {
    const exportPromise = new Promise(async (resolve, reject) => {
      try {
        if (!geojsonData) {
          return reject(new Error("地图数据尚未加载，无法导出。"));
        }
        ...
        if (sortedCities.length === 0) {
          return reject(new Error("没有包含日期和照片的城市可供导出。"));
        }
        
        sortedCities.sort((a, b) => new Date(a.visit_date) - new Date(b.visit_date));
    ...
    <div id="app-container">
      ...
      <div className="ui-top-left-cluster">
        <div className="user-info-bar">
          ...
          {/* We remove isExporting because toast.promise handles the loading state implicitly */}
